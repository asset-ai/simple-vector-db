...
static enum MHD_Result nearest_handler_callback(void* cls, struct MHD_Connection* connection,
    ...
    for (size_t i = 0; i < db->size; ++i) {
        if (db->vectors[i].dimension == vec.dimension) {
            double distance = fabs(db->vectors[i].median_point - vec.median_point);
    ...
    cJSON *vector_array = cJSON_CreateDoubleArray(db->vectors[nearest_index].data, db->vectors[nearest_index].dimension);
    cJSON_AddItemToObject(json_response, "vector", vector_array);
    cJSON_AddNumberToObject(json_response, "median_point", db->vectors[nearest_index].median_point);
